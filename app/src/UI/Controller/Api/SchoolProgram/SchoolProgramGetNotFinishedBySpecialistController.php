...
use App\UI\Controller\Api\SchoolProgram\Form\SchoolProgramGetNotFinishedBySpecialistForm;
...
/**
 * Class SchoolProgramGetNotFinishedBySpecialistController.
 *
 * @package App\UI\Controller\Api\Skill
 */
class SchoolProgramGetNotFinishedBySpecialistController extends AbstractController
{
    /**
    ...
    /**
     * SchoolProgramGetNotFinishedBySpecialistController constructor.
     *
     ...
     * @SWG\Tag(
     *     name="School Program",
     *     description="Получение незавершенных курсов для определенного специалиста."
     * )
     *
     * @SWG\Parameter(
     *     in="query",
     ...
     * ),
     *
     * @Route("api/v1/school-program_get-not-finished-by-specialist-id", methods={"GET"})
     *
    ...
    {
        $form = $this->createForm(SchoolProgramGetNotFinishedBySpecialistForm::class)
            ->submit($request->query->all());
        RequestFormValidationHelper::validate($form);

        try {
            $result = $this->schoolProgramGetService->getNotFinishedBySpecialistId(
                (int) $request->get('id'),
            );
